...
LalaTray::LalaTray(QObject *root, QObject *parent)
  ...
  connect(rootWin_, SIGNAL(newTitlePlaying(QString)), this,
          SLOT(onNewTitlePlaying(QString)));
  setContextMenu(trayIconMenu());

  connect(rootWin_, SIGNAL(isPlaying(bool)), this,
...
void LalaTray::onSetVolume(int val) {
  if (showVolumeMessage_) {
    showMessage("Volume " + QString::number(val) + "%", toolTip(),
                QSystemTrayIcon::NoIcon, 500);
  }
