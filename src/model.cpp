...
QString Model::getSortQueryString(const QString &title, const QString &genre,
...
QJsonObject Model::getMetadataForMrl(const QUrl &mrl) const {
  ...
  QJsonObject retval =
      Model::queryResultToJson(result).second.first().toObject();
  qDebug() << retval;
  return retval;
}
...
QStringList Model::getAlbumList(const QString &album, const QString &artist,
                                const QString &genre) {
  QStringList retval;
